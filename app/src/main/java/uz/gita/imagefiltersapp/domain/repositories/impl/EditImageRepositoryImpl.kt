...
class EditImageRepositoryImpl(private val context: Context) : EditImageRepository {
    ...
    override suspend fun saveFilteredImage(filteredImage: Bitmap): Uri? {
        return try {
            val mediaStorageFile = File(context.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "Saved Images")
            if(!mediaStorageFile.exists()){
                mediaStorageFile.mkdir()
            }
            val fileName = "img_${System.currentTimeMillis()}.jpg"
            val file = File(mediaStorageFile,fileName)
